...
#define LOWTHRESHOLD 0.70
#define HIGHTHRESHOLD 0.95
...
double FULL_CHARGE = 24520; //full capacity in mWh
...
double STATEVECTOR[] = {2.237430,2.420519,2.603608,2.786697,2.969786,3.152875,3.335964,3.519053,3.702142,3.885231,4.068320};
...
int state_encode(double * statearray, int numstates, double value)
{
	...
	int k;
	if(value<=statearray[0]) return 0;
	else if(value>=statearray[numstates]) return numstates-1;
	else {
		for(k=0;k<=numstates;k++)
			if(value<=statearray[k+1] && value >= statearray[k])
				return k;
		}
...
{			
	...
	//Uncomment for MDP with AVGRATECPI
	...
	int state_number = state_encode(STATEVECTOR, TOTALSTATES, avgcurrent_x_cpi);
	cout<<state_number<<" ";
	
	switch(state_number)
		{
			case 0: freqindex = 0; break ;
			case 1: freqindex = 3; break;
			case 2: freqindex = 1; break;
			case 3: freqindex = 3; break;
			case 4: freqindex = 0; break;
			case 5: freqindex = 2; break;
			case 6: freqindex = 2; break;
			case 7: freqindex = 1; break;
			case 8: freqindex = 2; break;
			case 9: freqindex = 2; break;
			default:  break;
		}
...
int main (int argc, char **argv)
{
	...
	//puts("CPI TOTCYC TOTINST dCYC dINST RATE avgRATE CAP DSCHRG VOLT avgVOLT avgCURRENT BDxCPI avgRATExCPI BUSYTIME TOTALTIME UTIL FREQ iniFREQINDEX CHANGE finFREQINDEX\n");	
	
	//For MDP with BDCPI
	puts("CPI TOTCYC TOTINST dCYC dINST DSCHRGRATE avgDSCHRGRATE CAP dCAP BATTDSCHRG VOLT avgVOLT avgCURRENT avgCURRENTxCPI BDxCPI avgRATExCPI INIFREQ STATE ACTION FINFREQ CORE0TEMP CORE1TEMP UTIL TIME\n");
