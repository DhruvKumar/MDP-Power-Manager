...
double FULL_CHARGE = 24430; //full capacity in mWh
...
int TOTALSTATES = 10; 
double STATEVECTOR[] = {0.018327, 0.131744, 0.245160, 0.358576, 0.471992, 0.585409, 0.698825, 0.812241, 0.925657, 1.039074, 1.152490};
int AVGn_DEPTH = 9;
 

double * AVGn_UTIL_ARRAY;

double AVGn_SCALER = 2.0/3.0;
int avgn_index =0;
int FLAG_AVGn_start =0;
...
int state_encode(double * statearray, int numstates, double value)
{
	// Encode the BDCPI or AVGRATECPI value into a state
	
	for(int k=0; k < numstates; k++)
		if( value >= statearray[k] && value <=statearray[k+1] )
			return k;
			
}  

double avgn_util(double * avgn_array, int avgn_depth, double avgn_scaler)
{
	// Returns the weighted average cpu utilization of last avgn_depth epochs
	
	int index = 0;
	double scaler = 0.0;
	double sum_cpu_util =0.0;
	for(index = 0; index < avgn_depth ; index++)
	{
		double power = avgn_depth-index;
		scaler = pow(avgn_scaler, power);
		sum_cpu_util += scaler * avgn_array[index]; 
	}
	return sum_cpu_util;
}
...
int cpu_utilization(void) //second print sequence
{
	...
	double cpu_util_avgn = 0.0;
	int current_avgn_index = 0; 
		...
		if(FLAG_AVGn_start == 0)
		{
		
		
		
			if((cpu_utilization <= LOWTHRESHOLD) && (freqindex >= 1)) 
			{
				 
				freqindex-=1;
			}
			else if ((cpu_utilization >= HIGHTHRESHOLD) && (freqindex <= 2)) 
			{
			...
			write_line(CURRENT_SPEED_FILE, "%u\n", FREQUENCYARRAY[freqindex]);
			
			cout<<"0"<<" "; //Dummy utilization for initial print sequence.
			
			cout<<FREQUENCYARRAY[freqindex];
		
			AVGn_UTIL_ARRAY[avgn_index]= cpu_utilization;
		
			avgn_index = avgn_index+1;
		
			if(avgn_index == AVGn_DEPTH) 
			{
		...
		else 
		{
			...
			// Get the average of last avgn_depth CPU Utilizations.
			cpu_util_avgn = avgn_util(AVGn_UTIL_ARRAY, AVGn_DEPTH, AVGn_SCALER);
			
			//Actual avgn CPU Utilization printed.
			cout << cpu_util_avgn<<" ";
		
			// Store the current CPU Utilization in appropriate cell of the array. 
			// cell 0 = t-9, 1= t-8....
			int shift_index;
			for(shift_index=0; shift_index < AVGn_DEPTH-1; shift_index++)
				AVGn_UTIL_ARRAY[shift_index] = AVGn_UTIL_ARRAY[shift_index + 1];
			
			AVGn_UTIL_ARRAY[AVGn_DEPTH-1] = cpu_utilization;
			
		
			if((cpu_util_avgn <= LOWTHRESHOLD) && (freqindex >= 1)) 
			{				
				freqindex-=1;
			}
			else if ((cpu_util_avgn >= HIGHTHRESHOLD) && (freqindex <= 2)) 
			{
				freqindex+=1;
			}
...
{			
	...
	if(battery_capacity_file.is_open())
	{
		while(!battery_capacity_file.eof())
		{
			...
			if (linecount == 6)
			{
				...
				avgcurrent_x_cpi = (double)(cpi*avg_rate)/avg_voltage;// figure of merit avgcurrent x cpi
				cout << avgcurrent_x_cpi << " "; 
			}
		}
	}
	...
	double cpi_x_bd = cpi*int_capacity/FULL_CHARGE;
	cout << cpi_x_bd << " ";
...
{
	AVGn_UTIL_ARRAY = new double [AVGn_DEPTH];
	int z;
	...
	//Uncomment the folliwng for PAST
	puts("CPI TOTCYC TOTINST dCYC dINST DSCHRGRATE avgDSCHRGRATE CAP dCAP BATTDSCHRG VOLT avgVOLT avgCURRENT avgCURRENTxCPI BDxCPI avgRATExCPI CORE0TEMP CORE1TEMP UTIL INIFREQ avgnUTIL FINFREQ TIME\n");	
	//puts("CPI avgRATE BD avgVOLT avgCURRENT BDxCPI avgRATExCPI  0 1 2 3 4 5 6 7 8 UTIL AVGn_UTIL\n");	
	
	//For MDP with BDCPI
	//puts("CPI TOTCYC TOTINST dCYC dINST RATE avgRATE CAP DSCHRG VOLT avgVOLT avgCURRENT BDxCPI INIFREQ STATE finFREQINDEX CPU0TEMP CPU1TEMP \n");
