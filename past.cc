...
double previous_busy_time = 0.0, previous_total_time = 0.0, inst_per_capacity = 0.0;
...
{			
	...
	double  inst_per_current, avg_current;
	...
	if(battery_capacity_file.is_open())
	{
		while(!battery_capacity_file.eof())
		{
			...
			if (linecount == 5)
			{
				...
				//cout << int_capacity << " " << int_capacity/FULL_CHARGE <<" ";
				if(!change_capacity == 0) inst_per_capacity = diff_inst/change_capacity;
				cout << change_capacity << " " << inst_per_capacity<<" "<< change_capacity/FULL_CHARGE <<" ";  /////////
			...
			if (linecount == 6)
			{
				...
				avg_current=(double)avg_rate/avg_voltage;
				inst_per_current = diff_inst/avg_current;
				cout << avg_current << " "<< inst_per_current<<" "; //average current drawn in Amperes I=P/V
	...
	// Implement the MDP DVS policy here.
	// Get states from matrix_calc.cc
...
{	
	...
	{		
		int fd; 
		fd = open("report", O_RDWR | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR | S_IXUSR);
...
int main (int argc, char **argv)
{
	...
	real_timer.it_interval.tv_sec = 5;
	real_timer.it_interval.tv_usec = 0; //5.0 seconds
	real_timer.it_value.tv_sec = 5;
	real_timer.it_value.tv_usec = 0;
	
	puts("CPI TOTCYC TOTINST dCYC dINST DSCHRGRATE avgDSCHRGRATE CAPACITY dCAPACITY INSTPERCAP BATTDSCHRG VOLT avgVOLT avgCURRENT INSTPERCURR avgCURRENTxCPI BDxCPI avgRATExCPI BUSYTIME TOTALTIME UTIL FREQ iniFREQINDEX CHANGE finFREQINDEX CORE0TEMP CORE1TEMP TIME\n");	
